...
def cos_similarity_cubed(
    clip_feats, target_feats, device="cuda", batch_size=10000, min_norm=1e-3
):
    """
    ...
    with torch.no_grad():
        ...
        clip_feats = clip_feats / torch.clip(
            torch.norm(clip_feats, p=2, dim=0, keepdim=True), min_norm
        )
        target_feats = target_feats / torch.clip(
            torch.norm(target_feats, p=2, dim=0, keepdim=True), min_norm
        )
        similarities = []
        for t_i in tqdm(range(math.ceil(target_feats.shape[1] / batch_size))):
            curr_similarities = []
            curr_target = (
                target_feats[:, t_i * batch_size : (t_i + 1) * batch_size].to(device).T
            )
            for c_i in range(math.ceil(clip_feats.shape[1] / batch_size)):
                curr_similarities.append(
                    curr_target
                    @ clip_feats[:, c_i * batch_size : (c_i + 1) * batch_size].to(
                        device
                    )
                )
...
def soft_wpmi_2(
    ...
    with torch.no_grad():
        ...
        prob_d_given_e = []
        p_in_examples = p_start - (
            torch.arange(start=0, end=top_k) / top_k * (p_start - p_end)
        ).unsqueeze(1).to(device)
        for orig_id in tqdm(range(target_feats.shape[1])):
            curr_clip_feats = clip_feats.gather(
                0, inds[:, orig_id : orig_id + 1].expand(-1, clip_feats.shape[1])
            ).to(device)
            # eq (10)
            curr_p_d_given_e = 1 + p_in_examples * (curr_clip_feats - 1)
            # eq (12)
            curr_p_d_given_e = torch.log(curr_p_d_given_e + min_prob)
            curr_p_d_given_e = torch.sum(curr_p_d_given_e, dim=0, keepdim=True)

            prob_d_given_e.append(curr_p_d_given_e)
            torch.cuda.empty_cache()
        prob_d_given_e = torch.cat(prob_d_given_e, dim=0)
        # logsumexp trick to avoid underflow
        prob_d = torch.logsumexp(prob_d_given_e, dim=0, keepdim=True) - torch.log(
            prob_d_given_e.shape[0] * torch.ones([1]).to(device)
        )
        mutual_info = prob_d_given_e - lam * prob_d

        pc_given_n = prob_d_given_e  # log scale
        norm = torch.logsumexp(
            prob_d_given_e, dim=1, keepdim=True
        )  # log scale normalizing factor
        pc_given_n = (pc_given_n - norm).exp().type(torch.float32)
    return mutual_info, pc_given_n
...
def wpmi(
    clip_feats, target_feats, top_k=28, a=2, lam=0.6, device="cuda", min_prob=1e-7
):
    with torch.no_grad():
        torch.cuda.empty_cache()
        clip_feats = torch.nn.functional.softmax(a * clip_feats, dim=1)
        ...
        for orig_id in tqdm(range(target_feats.shape[1])):
            ...
            curr_p_d_given_e = torch.sum(
                torch.log(curr_clip_feats + min_prob), dim=0, keepdim=True
            )
            prob_d_given_e.append(curr_p_d_given_e)
        prob_d_given_e = torch.cat(prob_d_given_e, dim=0)
        # logsumexp trick to avoid underflow
...
def rank_reorder(
    clip_feats, target_feats, device="cuda", p=3, top_fraction=0.05, scale_p=0.5
):
    """
    ...
    with torch.no_grad():
        ...
        for orig_id in tqdm(range(target_feats.shape[1])):
            clip_indices = clip_feats.gather(
                0, inds[:, orig_id : orig_id + 1].expand([-1, clip_feats.shape[1]])
            ).to(device)
            ...
            baseline_diff = sorted_target - torch.cat(
                [sorted_target[torch.randperm(len(sorted_target))] for _ in range(5)],
                dim=1,
            )
            ...
            for i in range(math.ceil(clip_indices.shape[1] / batch)):
                ...
                diff = target - reorg
                curr_errors.append(
                    torch.mean(torch.abs(diff) ** p, dim=0, keepdim=True)
                    / baseline_diff
                )
